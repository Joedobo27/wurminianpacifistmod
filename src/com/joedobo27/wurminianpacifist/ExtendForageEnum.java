...
class ExtendForageEnum {
    private static ArrayList<ExtendForageEnum.EnumData> toExtendEntries = new ArrayList<>();
    private static final String className = "com.wurmonline.server.behaviours.Forage";
    ...
    ExtendForageEnum(ClassPool classPool) throws NotFoundException{
        ExtendForageEnum.classPool = classPool;
        ExtendForageEnum.enumCtClass = ExtendForageEnum.classPool.get(className);
        ExtendForageEnum.enumClassFile = enumCtClass.getClassFile();
        ExtendForageEnum.enumConstPool = enumClassFile.getConstPool();
        ExtendForageEnum.initiatorMethodInfo = enumCtClass.getClassInitializer().getMethodInfo();
        ExtendForageEnum.initiatorCodeAttribute = initiatorMethodInfo.getCodeAttribute();
    ...
    private class EnumData {
        ...
        short action;
        //see behaviours.Actions. In Forage enum this is called "category".
        int item;
        // see item.ItemList. Aka, itemType.
        byte material;
        // if zero, then ItemFactory.createItem() will look up the template.getMaterial().
        // Further, I'm not sure why this here since every entry for Forage has a material of type 0.
        int chanceAt1Skill;
    ...
    static void createFieldsInEnum() throws  CannotCompileException {
        ...
        for (ExtendForageEnum.EnumData enumData : toExtendEntries) {
    ...
    private static void initiatorParser() throws BadBytecode {
        ...
        int constPoolValuesIndex = BytecodeTools.byteArrayToInt(BytecodeTools.findConstantPoolReference(enumConstPool,
                "// Field com/wurmonline/server/behaviours/Forage.$VALUES:[Lcom/wurmonline/server/behaviours/Forage;"), 2);
        codeIterator.begin();
        int lastIndex = 0;
    ...
    static void initiateEnumEntries() throws BadBytecode, ClassNotFoundException, NotFoundException {
        ...
        for (ExtendForageEnum.EnumData enumData : toExtendEntries) {
            ...
            enumInitiator.addPutstatic(className, enumData.fieldName, "Lcom/wurmonline/server/behaviours/Forage;");

            populateVALUES.addOpcode(Opcode.DUP);
            putInteger(populateVALUES, valuesSize + extensionCounter);
            extensionCounter++;
            populateVALUES.addGetstatic(className, enumData.fieldName, "Lcom/wurmonline/server/behaviours/Forage;");
